...
const Match = {
     ...
     */
    renderCreateMatch() {
        // Calculate a default match_time (next hour, rounded)
        const now = new Date();
                ...
                <div style="max-width:560px;">
                    <div class="card">
                        <form id="create-match-form">
                            ...
                            <div class="form-group" id="partner-search-group">
                                <label for="partner-search">Partner to Play With <span style="color:var(--danger)">*</span></label>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-hint">Team total will determine which opponents can join (±10 points rule)</div>
                            </div>
     ...
     */
    bindCreateMatch() {
        // Fetch and setup courts autocomplete
                    ...
                    html += `
                        <div class="player-search-item create-partner-item" data-id="${p.id}" data-name="${Utils.escape(p.name)}" data-points="${p.points}">
                            <div class="mini-avatar">
                                ${Utils.avatar(p.profile_image, 32, p.name, p.gender)}
                            </div>
        ...
        $('#create-match-form').on('submit', function (e) {
            ...
            if (!courtId) {
                Utils.toast('Please select a court', 'error');
                $btn.prop('disabled', false).html('🎾 Create Match');
                return;
            }
            if (!matchTime) {
                Utils.toast('Please set date & time', 'error');
                $btn.prop('disabled', false).html('🎾 Create Match');
                return;
            }
            if (!partnerId) {
                Utils.toast('You must select a partner to create a match', 'error');
                $btn.prop('disabled', false).html('🎾 Create Match');
                return;
            }
            if (!courtName) {
                Utils.toast('Please enter a court number or name', 'error');
                $btn.prop('disabled', false).html('🎾 Create Match');
                return;
            }
            ...
            Utils.api('match/create.php', 'POST', data).then(function (resp) {
                Utils.toast('Match created! Waiting for opponents.', 'success');
                ...
                <div class="card" style="margin-bottom:24px;padding:16px;">
                    <div style="display:grid;grid-template-columns:1fr;gap:16px" id="match-filters">
                        <div class="form-group" style="margin:0;position:relative">
                            <label style="font-size:0.85rem">Partner to Join With</label>
    ...
    renderFilteredOpenMatches() {
        ...
        state.matches.forEach(m => {
            ...
            if (m.is_participant) { m.can_join = false; m.reason = "🚫 You're joined."; }
            else if (!partnerPoints) { m.can_join = false; m.reason = "🚫 Partner to join"; }
            else if (m.points_diff > 10) { m.can_join = false; m.reason = `Points diff too high (${m.points_diff} pts apart)`; }
            else { m.can_join = true; m.reason = ''; }
            m.user_points = totalTeamPoints; // Provide for UI display context
        });
    ...
    showJoinModal(matchId, teamATotalPoints) {
        ...
        Utils.showModal('Join Match', `
            <p style="margin-bottom:16px">Join this match as Team B opponent.</p>
            <div class="form-group" style="position:relative">
                <label>Select Your Partner (Required)</label>
                <div class="form-hint mb-sm">Team A has ${teamATotalPoints} pts. You and your partner must be within ±10 points.</div>
        ...
        $('#join-partner-results').off('click', '.join-partner-item').on('click', '.join-partner-item', function () {
            ...
            $('#join-partner-info').html(`👥 <strong>${name}</strong> (${points} pts)`);
        ...
        $('#confirm-join-btn').on('click', function () {
            ...
            if (!pid) {
                Utils.toast('You must select a partner to join the match', 'error');
                return;
            }

            data.partner_id = parseInt(pid);
            $btn.prop('disabled', true).html('<span class="spinner"></span> Joining...');

            Utils.api('match/join.php', 'POST', data).then(function (resp) {
                Utils.toast('Successfully joined match!', 'success');
                Utils.closeModal();
    ...
    loadMatchDetails(matchId) {
        Utils.api(`match/details.php?id=${matchId}`).then(function (resp) {
            ...
            // Show action buttons based on match state
            if (m.status === 'open' && !m.is_participant && !m.waitlist?.find(w => w.is_mine)) {
